...
- T289 completed: added per-direct-client-IP fixed-window rate limits for anonymous booking submission (5/minute) and booking tracking (30/minute), with configured limits, `429` API envelopes, and `Retry-After` responses. Management and authenticated booking endpoints are not rate-limited by these public policies.
- T290 completed: added deny-by-default configured CORS, development support for the Angular local origin, standard browser security headers, restrictive API content security policy, and non-development HSTS. Added integration coverage for response headers and configured-origin CORS behavior.
- T300 completed: measured 107 unit-focused tests at 30.64% line and 30.44% branch coverage, documented the full-suite 59.08% line baseline for context, identified business-rule strengths, and prioritized untested domain query and rejection paths without expanding implementation scope.
- T301 completed: measured 66 HTTP integration tests at 51.56% line and 56.05% branch coverage, documented covered API behavior and prioritized gaps in booking transitions, HouseHelp mutations, production exception handling, and SQL Server-specific validation.
- T304 completed: reviewed EF Core query shapes and index coverage, documented no-tracking and existing-index strengths, and deferred composite-index changes until SQL Server execution-plan evidence is available. Identified bounded HouseHelp pagination and deterministic ordering as T306/T307 follow-up work.
- T305 completed: reviewed the HouseHelp-scoped booking overlap `EXISTS` query and its `(AssignedHouseHelpId, ScheduledStart, ScheduledEnd)` index alignment, preserving serializable transaction and lock safeguards. Documented SQL Server plan and workload evidence required before considering a filtered conflict index.
- T306 completed: standardized all collection queries on a default 50-item page and 100-item maximum, including previously unbounded HouseHelp, public service, system-setting, user, booking, notification, and audit-log lists. Added stable HouseHelp ordering and shared pagination coverage.
- T307 completed: added database-side active-state filtering for Admin services and exact role/active-state filtering for Admin users, preserving existing HouseHelp, booking, notification, and audit filtering. Added focused integration coverage and documented list-filter behavior.
- T308 completed: reviewed Serilog request correlation, configured enrichment/sinks, log levels, and sensitive-data handling. Added an allow-listed structured operational logging contract and documented the absence of a current operational requirement for new domain log events.
- T309 completed: verified the existing anonymous liveness endpoint and database-backed readiness endpoint, including their existing integration coverage, and documented their distinct probe contracts.
- T310 completed: reviewed production configuration and corrected JWT resolution so `JWT_KEY` takes precedence for both issuance and validation. Production now rejects missing, placeholder, and short signing keys; deployment configuration remains deferred to Phase 16.
- T311 completed: ran restore, Release build, and Release test pipeline successfully with 182 passing tests. Corrected the CI workflow to target the explicit test project and isolate its development signing-key fixture from the required CI production secret.
